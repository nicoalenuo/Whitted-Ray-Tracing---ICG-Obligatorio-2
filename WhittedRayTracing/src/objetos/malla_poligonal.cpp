...
malla_poligonal::~malla_poligonal() {
...
bool malla_poligonal::interseca(rayo rayo_in, vector_3& punto_interseccion_out) {
    ...
    float paralelicibilidad;

    for (size_t i = 0; i < poligonos.size(); i++) {
        triangulo = poligonos[i];
        normal = triangulo->normal();
        paralelicibilidad = normal.producto_interno(rayo_in.get_direccion());
        if (paralelicibilidad != 0) {
            float distancia = -(normal.producto_interno(rayo_in.get_origen()) - normal.producto_interno(triangulo->get_coord_1())) / paralelicibilidad;

            if (distancia > 0) {
                vector_3 punto_interseccion = rayo_in.get_origen() + rayo_in.get_direccion() * distancia;

                vector_3 arista_1 = triangulo->get_coord_2() - triangulo->get_coord_1();
                vector_3 arista_2 = triangulo->get_coord_3() - triangulo->get_coord_2();
                vector_3 arista_3 = triangulo->get_coord_1() - triangulo->get_coord_3();
